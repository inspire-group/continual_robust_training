...
def ycbcr_to_rgb_jpeg(image):
    ...
    shift_yuv = torch.FloatTensor([0, -128, -128]).to(device)
...
def rgb_to_ycbcr_jpeg(image):
    ...
    shift_rgb = torch.as_tensor([0.0, 128.0, 128.0], device=device)
...
class JPEGAdversary(nn.Module):
    ...
    edited from the original implementation in XXXX.
    ...
    def forward(self, model, inputs, targets):
        ...
        if self.task == "var_reg":
            noise_vars_2 = self.epsilon * torch.rand(
                (batch_size, num_channels, height // 8 * width // 8, 8, 8)
            ).to(device)
            noise_vars_2.requires_grad = True
        ...
        for _ in range(self.num_steps):
            ...
            if self.task == "attack":
                ...
                adv_inputs_2 = apply_jpeg(inputs.detach(), noise_vars_2)
                loss = l2_loss(outputs, model(adv_inputs_2))

            if self.task == "var_reg":
                grads, grads2 = torch.autograd.grad(loss, [noise_vars, noise_vars_2])
            else:
            ...
            if self.task == "var_reg":
                noise_vars_2 = noise_vars_2 + self.step_size * torch.sign(grads2)
                noise_vars_2 = torch.clamp(noise_vars_2, -self.epsilon, self.epsilon)
...
class JPEGAttackBase(AttackInstance):
...
class JPEGRegBase(AttackInstance):

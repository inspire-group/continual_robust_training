...
def get_random_circle(image_size, radius):
    ...
    xx, yy = torch.meshgrid(
        torch.arange(width).to(device),
        torch.arange(height).to(device),
        indexing="xy",
    )
...
def get_random_equilateral_triangle(image_size, edge_size):
    ...
    batch_size, num_channels, height, width = image_size
    triangle_mesh = torch.zeros((height, width), dtype=bool).to(device)
...
def fill_triangle(p1, p2, p3, fill_tensor):
    ...
    # filled triangle routines ported From XXXX
    if p2[1] == p3[1]:
...
def get_random_square(image_size, edge_size):
    batch_size, num_channels, height, width = image_size
    square_mask = torch.zeros((height, width), dtype=bool).to(device)
...
def get_shape_masks(image_size, num_shapes, shape_size):
    ...
    for _ in range(image_size[0]):
        ...
        for _ in range(0, num_shapes):
            ...
            shape_masks.append(shape_mask.to(device))
...
def init_colours(batch_size, num_shapes, min_value_valence, min_value_saturation):
    colours = torch.zeros((batch_size, 3, num_shapes, 1)).to(device)

    colours[:, 0:1, :, 0:1] = torch.rand((batch_size, 1, num_shapes, 1)).to(
        device
    )
    colours[:, 1:2, :, 0:1] = (
        torch.rand((batch_size, 1, num_shapes, 1)).to(device)
...
def get_edge_masks(shape_mask, edge_width):
    ...
    kernels = torch.ones(num_shapes, 1, edge_width, edge_width).to(device)
...
class KaleidoscopeAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        ...
        if self.same_color:
            colour_vars = (
                (torch.rand(size=colour_init.size()).to(device) - 0.5)
                * self.epsilon
                * 2
            )
            if self.task == "var_reg":
                ...
                colour_vars_2.requires_grad =True
        else:
            colour_vars = (
                (torch.rand(size=inputs.shape).to(device) - 0.5)
                * self.epsilon
                * 2
            )
            if self.task == "var_reg":
        ...
        edge_vars = (
            torch.rand(size=(batch_size, 1, height, width)).to(device)
            * self.epsilon
        )
        ...
        if self.task == "var_reg":
            ...
            edge_vars_2.requires_grad = True
        ...
        for k in range(self.num_steps):
            ...
            if self.task == "attack":
                ...
                loss = F.cross_entropy(outputs, clean_out)
            else:
                adv_inputs_2 = add_variables(
                    inputs,
                    shape_mask,
                    colour_vars_2,
                    edge_mask,
                ...
                loss = F.cross_entropy(outputs, model(adv_inputs_2))

            if self.task == "var_reg":
                colour_grads, edge_grads, colour_grads_2, edge_grads_2 = torch.autograd.grad(
                    loss, [colour_vars, edge_vars, colour_vars_2, edge_vars_2]
                )
            else:
            ...
            if self.task == "var_reg":
                colour_vars_2 = colour_vars_2 + self.step_size * torch.sign(colour_grads_2)
                colour_vars_2 = torch.clamp(colour_vars_2, -self.epsilon, self.epsilon)

                colour_vars_2 = colour_vars_2.detach()
                colour_vars_2.requires_grad = True

                edge_vars_2 = edge_vars_2 + self.step_size * torch.sign(edge_grads_2)
                edge_vars_2 = torch.clamp(edge_vars_2, -self.epsilon, self.epsilon)

                edge_vars_2 = edge_vars_2.detach()
                edge_vars_2.requires_grad = True
...
class KaleidoscopeAttackBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, kaleidoscope_num_shapes, kaleidoscope_shape_size,
    kaleidoscope_min_value_valence, kaleidoscope_min_value_saturation, kaleidoscope_transparency, kaleidoscope_edge_width):
        ...
        self.attack = KaleidoscopeAdversary(
            ...
            num_steps=num_steps,
            step_size=step_size,
            num_shapes=kaleidoscope_num_shapes,
            shape_size=kaleidoscope_shape_size,
            min_value_valence=kaleidoscope_min_value_valence,
            min_value_saturation=kaleidoscope_min_value_saturation,
            transparency=kaleidoscope_transparency,
            edge_width=kaleidoscope_edge_width,
        )
...
class KaleidoscopeRegBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, kaleidoscope_num_shapes, kaleidoscope_shape_size,

...
from .attacks import AttackInstance, normalize_l2, tensor_clamp_l2
...
class CannyFilter(nn.Module):
    """
    Implements the "Canny edge detector" as a PyTorch module. See XXXX for
    details on the algorithm. Implementatation is based on the one here XXXX.
    """
...
class TextureAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, distance_metric, threshold, task):
        super().__init__()
    ...
    def forward(self, model, inputs, targets):
        ...
        edge_image = torch.ones(inputs.size()).to(device)
        ...
        if self.distance_metric == "l2":
            interp_vars = (
                ...
                * self.normalise_tensor(
                    torch.rand(batch_size, height, width, device=device)
                )
                * random.random()
            )
        ...
        for i in range(self.num_steps):
            ...
            if self.task == "attack":
                ...
                adv_inputs_2 = dynamic_interpolation(inputs, edge_image, interp_vars_2, edges)
                loss = l2_loss(logits, model(adv_inputs_2))

            # Typical PGD implementation

            if self.task == "var_reg":
                grad, grad2 = torch.autograd.grad(loss, [interp_vars, interp_vars_2])
            else:
            ...
            if self.task == "var_reg":
                grad2 = self.normalise_tensor(grad2)

                interp_vars_2 = interp_vars_2 + self.step_size * grad2
                interp_vars_2 = self.project_tensor(interp_vars_2, self.epsilon)
                interp_vars_2 = torch.clamp(interp_vars_2, -1, 1)

                interp_vars_2 = interp_vars_2.detach()
                interp_vars_2.requires_grad = True
...
class TextureAttackBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, distance_metric, texture_threshold):
...
class TextureRegBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, distance_metric, texture_threshold, task):
        super().__init__(model, 0)
        self.attack = TextureAdversary(
            epsilon=epsilon,
            num_steps=num_steps,

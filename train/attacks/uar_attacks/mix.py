...
def get_mix_images(images, targets):
    ...
    returned_target_indices = torch.arange(batch_size).to(device)
...
class MixAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        a_start = time.time()
        inputs, targets = (
            inputs.to(device).detach(),
            targets.to(device).detach(),
        )
        """
        ...
        interp_matrix = self.epsilon * self.normalise_tensor(
            torch.rand(
                (batch_size, 1, height, width), device=device, requires_grad=True
            )
        )

        if self.task == "var_reg":
            interp_matrix_2 = self.epsilon * self.normalise_tensor(
                torch.rand(
                    (batch_size, 1, height, width), device=device, requires_grad=True
                )
            )
        ...
        for i in range(self.num_steps):
            ...
            if self.task == "var_reg":
                grad, grad2 = torch.autograd.grad(loss, [interp_matrix, interp_matrix_2], only_inputs=True)
            else:
            ...
            if self.task == "var_reg":
                interp_matrix_2 = interp_matrix_2 + self.step_size * grad2
                interp_matrix_2 = self.project_tensor(interp_matrix_2, self.epsilon)
...
class MixAttackBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, distance_metric, mix_interp_kernel_size, mix_interp_kernel_sigma):
        ...
        self.attack = MixAdversary(
            ...
            interp_kernel_size=mix_interp_kernel_size,
            interp_kernel_sigma=mix_interp_kernel_sigma,
        )
...
class MixRegBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, distance_metric, mix_interp_kernel_size, mix_interp_kernel_sigma, task):
        super().__init__(model, 0)
        self.attack = MixAdversary(
            epsilon=epsilon,
            num_steps=num_steps,

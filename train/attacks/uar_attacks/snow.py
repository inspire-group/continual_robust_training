...
# Pretty ad-hoc line creation algorithm, taken from XXXX58#47381058
...
def apply_snow(img, snow, discolor):
    ...
    """
    snow = snow.to(device).unsqueeze(1)
...
class SnowAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        ...
        mask = get_location_mask(flake_intensities, self.grid_size).to(device)
        ...
        if self.task == "var_reg":
            ...
            flake_intensities_2 = self.project_tensor(flake_intensities_2, self.epsilon)
            mask_2 = get_location_mask(flake_intensities_2, self.grid_size).to(device)

            flake_intensities_2 = flake_intensities_2 * mask_2

            # create initial variables
            flake_intensities_2.requires_grad_()
        ...
        for i in range(self.num_steps):
            ...
            if self.task == "attack":
                ...
                snow_noise_2 = make_snow(
                    flake_intensities_2, mask_2, kernels, self.normalizing_constant
                )
                adv_inputs_2 = apply_snow(inputs.detach(), snow_noise_2, self.image_discolour)
                loss = l2_loss(logits, model(adv_inputs_2))

            if self.task == "var_reg":
                grad, grad2 = torch.autograd.grad(loss, [flake_intensities, flake_intensities_2], only_inputs=True)
            else:
            ...
            if self.task == "var_reg":
                ...
                flake_intensities_2 = flake_intensities_2 + self.step_size * grad2
                flake_intensities_2 = self.project_tensor(flake_intensities_2, self.epsilon)

                flake_intensities_2 = flake_intensities_2.detach()
                flake_intensities_2.requires_grad_()
...
class SnowAttackBase(AttackInstance):
    def __init__(self, model, epsilon, distance_metric, num_steps, step_size, snow_flake_size, snow_num_layers,
    ...
    snow_sigma_range_lower, snow_sigma_range_upper):
        ...
        self.attack = SnowAdversary(
            ...
            num_steps=num_steps,
            step_size=step_size,
            flake_size=snow_flake_size,
            num_layers=snow_num_layers,
            grid_size=snow_grid_size,
            snow_init=snow_init,
            image_discolour=snow_image_discolour,
            normalising_constant=snow_normalizing_constant,
            gaussian_kernel_size=snow_kernel_size,
            sigma_range=(snow_sigma_range_lower, snow_sigma_range_upper),
        )
        self.model = model
...
class SnowRegBase(AttackInstance):
    def __init__(self, model, epsilon, distance_metric, num_steps, step_size, snow_flake_size, snow_num_layers,
    snow_grid_size, snow_init, snow_image_discolour, snow_normalizing_constant, snow_kernel_size,

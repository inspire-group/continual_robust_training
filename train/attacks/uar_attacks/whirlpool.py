...
class WhirlpoolAdversary(nn.Module):
    ...
    def forward(self, model, inputs, targets):
        ...
        if self.distance_metric == "l2":
            whirlpool_strength_vars = (
                ...
                * self.normalise_tensor(
                    torch.rand(batch_size, self.num_whirlpools, device=device)
                )
                * random.random()
            )
            if self.task == "var_reg":
                whirlpool_strength_vars_2 = (
                    self.epsilon
                    * self.normalise_tensor(
                        torch.rand(batch_size, self.num_whirlpools, device=device)
                    )
                ...
                whirlpool_strength_vars_2.requires_grad = True
        if self.distance_metric == "linf":
            whirlpool_strength_vars = self.epsilon * torch.rand(
                batch_size, self.num_whirlpools, device=device
            )
            if self.task == "var_reg":
                whirlpool_strength_vars_2 = self.epsilon * torch.rand(
                    batch_size, self.num_whirlpools, device=device
                )
                whirlpool_strength_vars_2.requires_grad = True
        ...
        whirlpool_centers = (
            torch.rand(batch_size, self.num_whirlpools, 2, device=device) - 0.5
        ) * 2
        ...
        for _ in range(0, self.num_steps):
            ...
            if self.task == "var_reg":
                ...
                adv_inputs_2 = swirl_image(
                    inputs,
                    whirlpool_centers,
                    torch.abs(whirlpool_strength_vars_2),
                    self.whirlpool_radius,
                    self.whirlpool_min_strength,
                )
            ...
            if self.task == "var_reg":
                grad, grad2 = torch.autograd.grad(loss, [whirlpool_strength_vars, whirlpool_strength_vars_2], only_inputs=True)
            else:
                grad = torch.autograd.grad(loss, whirlpool_strength_vars, only_inputs=True)[0]
            ...
            if self.task == "var_reg":
                ...
                whirlpool_strength_vars_2 = self.project_tensor(
                    whirlpool_strength_vars_2.unsqueeze(1), self.epsilon
                ).squeeze(1)
                whirlpool_strength_vars_2 = whirlpool_strength_vars_2.detach()
                whirlpool_strength_vars_2.requires_grad = True
...
class WhirlpoolAttackBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, distance_metric, num_whirlpools,
     whirlpool_radius, whirlpool_min_strength):
        ...
        self.attack = WhirlpoolAdversary(
            ...
            num_whirlpools=num_whirlpools,
            whirlpool_radius=whirlpool_radius,
            whirlpool_min_strength=whirlpool_min_strength,
        )
        self.model = model
...
class WhirlpoolRegBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, distance_metric, num_whirlpools,

...
class EdgeAdversary(nn.Module):
    ...
    def __init__(self, epsilon, num_steps, step_size, threshold, task="attack"):
        super().__init__()
    ...
    def forward(self, model, inputs, targets):
        ...
        edge_vars = torch.rand(size=inputs.shape).to(device)
        edge_vars[~edge_mask] = 0.0
        edge_vars.requires_grad = True

        if self.task == "var_reg":
            edge_vars_2 = torch.rand(size=inputs.shape).to(device)
            edge_vars_2[~edge_mask] = 0.0
        ...
        for i in range(self.num_steps):
            ...
            if self.task == "var_reg":
                adv_inputs_2 = apply_mask(inputs, edge_mask, edge_vars_2)
            ...
            if self.task == "var_reg":
                grad, grad2 = torch.autograd.grad(loss, [edge_vars, edge_vars_2])
            else:
            ...
            if self.task == "var_reg":
                ...
                edge_vars_2 = edge_vars_2 + self.step_size * grad2
...
class EdgeAttackBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, edge_threshold):
...
class EdgeRegBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, edge_threshold,task):
        super().__init__(model, 0)
        self.attack = EdgeAdversary(
            epsilon=epsilon,
            num_steps=num_steps,

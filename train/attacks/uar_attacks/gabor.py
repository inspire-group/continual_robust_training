...
def create_gabor_kernel(batch_size, k_size, sigma, sides):
    ...
    kernels = []
    theta = np.pi * torch.rand(1).to(device)
    gamma = ((k_size / 4.0 - 3) * torch.rand(1) + 3).to(device)
...
class GaborAdversary(nn.Module):
    """This class implements a gabor-noise based adversary. Gabor noise (explained in more detail here XXXX), is
    ...
    def forward(self, model, inputs, targets):
        ...
        if self.task == "var_reg":
            ...
            gabor_vars_2 = self.normalise_tensor(gabor_vars_2) * self.epsilon * random.random()
            mask = (gabor_vars_2 != 0).float()
            gabor_vars_2.requires_grad = True
        ...
        for i in range(self.num_steps):
            ...
            if self.task == "attack":
                ...
                gabor_noise_2 = generate_random_distribution(
                    gabor_vars_2,
                    mask,
                    inputs,
                    batch_size,
                ...
                loss = l2_loss(logits, model(inputs + gabor_noise_2))

            if self.task == "var_reg":
                grad, grad2 = torch.autograd.grad(loss, [gabor_vars, gabor_vars_2])
            else:
            ...
            if self.task == "var_reg":
                gabor_vars_2 = gabor_vars_2 + self.step_size * self.normalise_tensor(grad2)

                gabor_vars_2 = self.project_tensor(gabor_vars_2, self.epsilon)
                gabor_vars_2 = gabor_vars_2.detach()
                gabor_vars_2.requires_grad = True
        ...
        if self.task != "attack":
            if self.task == "var_reg":
                ...
                adv_inputs_2 = inputs + gabor_noise_2
...
class GaborAttackBase(AttackInstance):
    ...
    gabor_kernel_size, gabor_sides, gabor_sigma, gabor_weight_density):
        ...
        self.attack = GaborAdversary(
            ...
            sides=gabor_sides,
            sigma=gabor_sigma,
            weight_density=gabor_weight_density,
        )
        self.model = model
...
class GaborRegBase(AttackInstance):
    def __init__(self, model, epsilon, num_steps, step_size, distance_metric,

from .uar_attacks import *
from torch import nn
### from new UAR code base: XXXX

class BlurAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound=0.3, **kwargs):
        super().__init__()
        ...
        self.base_attack = BlurAttackBase(model, bound, num_iterations, step_size, blur_kernel_size,
    blur_kernel_sigma, blur_interp_kernel_size, blur_interp_kernel_sigma)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class EdgeAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound=0.1, **kwargs):
        super().__init__()
        step_size = 0.02
        num_iterations = 60
        edge_threshold = 0.1
        ...
        self.base_attack = EdgeAttackBase(model, bound, num_iterations, step_size, edge_threshold)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class ElasticAttack(nn.Module):
    def __init__(self, model, dataset_name = "imagenet", bound=0.25, **kwargs):
        super().__init__()
        ...
        self.base_attack = ElasticAttackBase(model, bound, num_iterations, step_size, elastic_upsample_factor)
    
    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class FBMAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.04, **kwargs):
        super().__init__()
        step_size = 0.06
        num_iterations = 30
        ...
        self.base_attack = FbmAttackBase(model, bound, num_iterations, step_size)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class FogAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.4, **kwargs):
        super().__init__()
        step_size = 0.05
        if dataset_name == "cifar":
            num_iterations = 40
        ...
        self.base_attack = FogAttackBase(model, bound, num_iterations, step_size, distance_metric, fog_wibbledecay)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class GaborAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.03, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            gabor_sides = 3
            num_iterations = 80
        ...
        self.base_attack = GaborAttackBase(model, bound, num_iterations, step_size, distance_metric,
    gabor_kernel_size, gabor_sides, gabor_sigma, gabor_weight_density)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class GlitchAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.05, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            glitch_num_lines = 16
            step_size = 0.0025
        ...
        self.base_attack = GlitchAttackBase(model, bound, num_iterations, step_size, glitch_num_lines, glitch_grey_strength)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class HSVAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.02, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            step_size = 0.003
            num_iterations = 20
        ...
        self.base_attack = HSVAttackBase(model, bound, num_iterations, step_size, distance_metric, 
        hsv_kernel_size, hsv_sigma)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class JPEGAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 3/255, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            num_iterations = 50
        else:
        ...
        self.base_attack = JPEGAttackBase(model, bound, num_iterations, step_size)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class KaleidoscopeAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.1, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
        ...
        self.base_attack = KaleidoscopeAttackBase(model, bound, num_iterations, step_size, kaleidoscope_num_shapes, kaleidoscope_shape_size,
    kaleidoscope_min_value_valence, kaleidoscope_min_value_saturation, kaleidoscope_transparency, kaleidoscope_edge_width)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class KlotskiAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.05, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            step_size = 0.005
        ...
        if "distance_metric" in kwargs:
            distance_metric = kwargs["distance_metric"]
        self.base_attack = KlotskiAttackBase(model, bound, num_iterations, step_size, distance_metric, klotski_num_blocks)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class MixAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 5, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            step_size = 0.5
            num_iterations = 30
        ...
        if "distance_metric" in kwargs:
            distance_metric = kwargs["distance_metric"]
        self.base_attack = MixAttackBase(model, bound, num_iterations, step_size, distance_metric, mix_interp_kernel_size, mix_interp_kernel_sigma)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class PixelAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 5, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            pixel_size = 4
        ...
        if "distance_metric" in kwargs:
            distance_metric = kwargs["distance_metric"]
        self.base_attack = PixelAttackBase(model, bound, num_iterations, step_size, pixel_size, distance_metric)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class PolkadotAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 2, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            num_iterations = 40
        else:
        ...
        if "distance_metric" in kwargs:
            distance_metric = kwargs["distance_metric"]
        self.base_attack = PolkadotAttackBase(model, bound, num_iterations, step_size, distance_metric, 
    polkadot_num_polkadots, polkadot_distance_scaling, polkadot_image_threshold, polkadot_distance_normaliser)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class PrisonAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.03, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            prison_num_bars = 8
        ...
        self.base_attack = PrisonAttackBase(model, bound, num_iterations, step_size, prison_num_bars, prison_bar_width)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class SnowAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 4, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            snow_flake_size = 3
            snow_num_layers = 5
        ...
        self.base_attack = SnowAttackBase(model, bound, distance_metric, num_iterations, step_size, snow_flake_size, snow_num_layers,
    snow_grid_size, snow_init, snow_image_discolour, snow_normalizing_constant, snow_kernel_size,
    snow_sigma_range_lower, snow_sigma_range_upper)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class TextureAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.1, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            step_size = 0.003
        ...
        self.base_attack = TextureAttackBase(model, bound, num_iterations, step_size, distance_metric, texture_threshold)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class WhirlpoolAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 100, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
            step_size = 16
            num_iterations = 50
        ...
        self.base_attack = WhirlpoolAttackBase(model, bound, num_iterations, step_size, distance_metric, num_whirlpools,
     whirlpool_radius, whirlpool_min_strength)

    def forward(self, x, y):
        return self.base_attack.generate_attack((x, y))

class WoodAttack(nn.Module):
    def __init__(self, model, dataset_name = "cifar", bound = 0.05, **kwargs):
        super().__init__()
        if dataset_name == "cifar":
        ...
        self.base_attack = WoodAttackBase(model, bound, num_iterations, step_size, wood_noise_resolution, wood_num_rings, wood_random_init, wood_normalising_constant)
